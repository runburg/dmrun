...
for fil in rho_dict.keys():
    try:
        os.mkdir(fil + '_vectorized')
    except FileExistsError:
        pass
    print('hi')
    os.chdir(fil + '_vectorized')
    f = open('output.txt', 'w')
    # sys.stdout = f
    ...
    if PLOT is True:
        fig, ax = plt.subplots()
        ax.plot(phi_vals + 1, sec_derv)
        ax.set_xscale('log')
        ax.set_yscale('log')
        fig.savefig('./secderv.pdf')

    print('sec derv', sec_derv[:5], sec_derv[-5:])

    ####
    integrand = np.sqrt(np.abs((phi_vals[np.newaxis, :].astype(np.float128)*(1-1e-9) - phi_vals[:, np.newaxis])))
    print(integrand)
    integrand[-1, -1] = 1
    integrand = 1 / (np.sqrt(8) * np.pi**2) * sec_derv[np.newaxis, :] / integrand.astype(np.float128)*np.tri(len(phi_vals)).T
    fval = integrate.simps(integrand, phi_vals, axis=-1)
    print(np.sum(np.isnan(fval)), 'nans in fvals')
    print(fval[:20])
    fvals = np.abs(np.nan_to_num(fval))
    ...
    recovered_rho_new = integrate.simps(4*np.sqrt(2)*np.pi*fvals[:, np.newaxis]*np.sqrt((phi_vals[:, np.newaxis]-phi_vals[np.newaxis, :]).T*np.tri(len(phi_vals)).T), phi_vals, axis=0)
    ...
    if PLOT is True:
        ...
        ax.plot(oldrs, recovered_rho_old, label='rho from kim fe', ls='-')
        ...
        ax.plot(r_vals, (recovered_rho_new - rho(r_vals))/rho(r_vals), label='rho from our fe')
        ax.set_xscale('log')
        ax.set_yscale('log')
        ax.set_xlabel('r')
        ...
        ax.plot(np.abs(phi_vals), fvals, ls='--', label='new fe')
        ax.plot(np.abs(phi_vals), 3/16/np.pi/np.sqrt(2) * (1 - phi_vals)**(-5/2), label='analytic solution', ls='-.')
        # ax.set_xscale('log')
        ax.set_yscale('log')
        ax.set_xlabel('E')
        ax.set_ylabel('f(E)')
        ...
        print(phi_vals[3], phi_vals[-3])
        es = np.abs(phi_vals + phi_vals[0])
        es = np.abs(np.flip(phi_vals))
        es = np.flip(-phi_vals) + 1
        # es = np.flip(phi_vals[3:-3]) + 1
        ax2.plot(es , np.abs(np.gradient(np.flip(fvals), es)), label='f(E) slope')
        ax2.plot(es, np.abs(-5/2 * 3/16/np.sqrt(2) / np.pi * es**(-7/2)), ls='--', label='Analytic solution')
        ax2.set_xscale('log')
        ax2.set_xlabel('E')
    ...
    vvals = np.logspace(-6, np.log10(np.sqrt(-2*phi_vals[0])), num=lim + 1)
    ...
    if PLOT is True:
        ...
        plt.plot(r_vals, 3 * np.pi / 16 * r_vals**(-2.5) / (32 * np.pi**2), ls='--', label='analytic')
        plt.xscale('log')
        plt.yscale('log')
        plt.xlabel('r')
        plt.ylabel('p2 som')
